...
/* harmony export */ });
const chatGptRequest = async (mainMessageList, token, stream, maxTokens = 30)=>{
    const headers = {
...
async function handler(req, res) {
    ...
    if (req.method === "POST") {
        ...
        console.log("\n");
        try {
            const response = await chatGptRequest(mainMessageList, process.env.OPENAI_API_KEY, stream, maxTokens);
            if (stream) {
                // Set up SSE headers
...
var __webpack_exports__ = (__webpack_exec__(7742));
